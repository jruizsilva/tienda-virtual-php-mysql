...
function openModal() {
  ...
  d.querySelector(".modal-header").classList.replace("headerUpdate", "headerRegister")
  d.querySelector("#btnActionForm").classList.replace("btn-info", "btn-primary")
...
function insertRole(formData) {
  axios.post(base_url + '/roles/insert', formData)
    .then(res => {
      console.log(res)
...
function updateRole(formData) {
  axios.post(base_url + '/roles/update', formData)
    .then(res => {
      console.log(res)
      if (res.data.success === true) {
        $('#role_modal').modal('hide');
...
function handleButtonEditRole(roleId) {
  ...
  d.querySelector(".modal-header").classList.replace("headerRegister", "headerUpdate")
  d.querySelector("#btnActionForm").classList.replace("btn-primary", "btn-info")
  d.querySelector("#btnText").textContent = "Actualizar"
  $button = this;
  axios.get(`${base_url}/roles/find/${roleId}`)
    .then(res => {
      ...
      if (res.data.success === true) {
        ...
        if ($role.status == 1) {
          ...
          optionSelect = '<option value="1" selected class="notBlock">Inactivo</option>';
        }
...
function handleButtonDeleteRole(roleId) {
  swal({
    ...
    if (isConfirm) {
      axios.post(`${base_url}/roles/delete/${roleId}`)
        .then((res) => {
          if (res.data.success === true) {
        ...
        .catch((err) => {
          const errorMessage = err.response.data.message ?? "Error al eliminar rol";
          swal("Error!", errorMessage, "error");
          console.log(err)
        })
    }
...
function handleButtonPermissionsRole(roleId) {
  axios.get(`${base_url}/permissions/all/roles/${roleId}`)
    .then(res => {
      if (res.data.success === true) {
        d.querySelector("#ajaxContent").innerHTML = res.data.html;
        $('.permissions_modal').modal('show')
        d.querySelector("#formPermissions").addEventListener('submit', handleSubmitPermissionsRole)
      }
    })
    .catch(err => {
...
function handleSubmitPermissionsRole(e) {
  ...
  axios.post(`${base_url}/permissions/update/roles/${roleId}`, formData)
    .then(res => {
      if (res.data.success === true) {
    ...
    .catch(err => {
      const errorMessage = err.response.data.message ?? "Error al asignar permisos";
      swal('Error', errorMessage, 'error');
      console.log(err)
    })
}
