...
function handleButtonViewUser(userId) {
  ...
  axios.get(base_url + '/users/find/' + userId)
    .then(res => {
      console.log(res)
      if (res.data.success === true) {
        const user = res.data.data;
        const userStatusHtml = Number(user.status) === 1 ?
...
function handleButtonDeleteUser(userId) {
  swal({
    ...
    if (isConfirm) {
      axios.post(`${base_url}/users/delete/${userId}`)
        .then((res) => {
          if (res.data.success === true) {
...
function fetchRolesOptions() {
  axios.get(base_url + "/roles/allSelectOptions")
    .then(res => {
      if (res.data.success == true) {
...
function insertUser(formData) {
  axios.post(base_url + '/users/insert', formData)
    .then(res => {
      console.log(res)
      if (res.data.success === true) {
...
function updateUser(formData) {
  axios.post(base_url + '/users/update', formData)
    .then(res => {
      console.log(res)
      if (res.data.success === true) {
        $('#users_modal').modal('hide');
      ...
      const errorMessage = err.response.data.message ?? "Error al editar usuario";
      swal('Error', errorMessage, 'error');
      console.log(err)
    })
}
function handleSubmit(e) {
  ...
  if (identificacion == '' || names == '' || surname == '' || phone == '' || email == '' || roleId == '') {
